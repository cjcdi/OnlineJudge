...
<html lang="en">
    ...
    <div class="container">
		...
		<script src="include/checksource.js"></script>
			<form id=frmSolution action="submit_fill.php" method="post" onsubmit='do_submit()'>
				<input type='hidden' value='<?php echo $problem_flag?>' name="problem_flag">
				<?php if (isset($id)){ //非竞赛?>
					Problem <span class=blue><b><?php echo $id?></b></span>
					<input id=problem_id type='hidden' value='<?php echo $id?>' name="id" ><br>
				<?php }else{ //$PID="ABCDEFGHIJKLMNOPQRSTUVWXYZ"; //if ($pid>25) $pid=25; ?>
					Problem <span class=blue><b><?php echo chr($pid+ord('A'))?></b></span>
					of Contest <span class=blue><b><?php echo $cid?></b></span><br> <?php //ord — 返回字符的 ASCII 码值?>
					<input id="cid" type='hidden' value='<?php echo $cid?>' name="cid">
					<input id="pid" type='hidden' value='<?php echo $pid?>' name="pid">
				<?php }?>	

				<span id="language_span"><?php /*Language:
					...
					*/?>
					[<b>当提交有多个答案时，每个答案占一行--不要输出其他任何多余的字符，包括换行和空格</b>]
					<?php if($OJ_VCODE){?>
						<?php echo $MSG_VCODE?>:
						<input name="vcode" size=4 type=text>
						<img id="vcode" alt="click to change" onclick="this.src='vcode.php?'+Math.random()">
					<?php }?><br>
				</span>
				...
				<?php /*if (isset($OJ_TEST_RUN)&&$OJ_TEST_RUN){?>
					...
					<?php echo $MSG_Output?>:<textarea style="width:30%" cols=10 rows=5 id="out" name="out" disabled="true" >SHOULD BE: <?php echo $view_sample_output?></textarea><br>
				<?php }*/ ?>
	...
	<script>
		...
		function do_submit(){
			...
			if(mark=='problem_id')
				...
				problem_id.value='<?php if (isset($cid)) echo $cid?>';
			document.getElementById("frmSolution").target="_self"; //_self - 在相同的框架或窗口中载入目标文档
			document.getElementById("frmSolution").submit();
		}

		var handler_interval;
		function do_test_run(){
			if( handler_interval) window.clearInterval( handler_interval); //用以指定在一段特定的时间后执行某段程序。
			var loader="<img width=18 src=image/loader.gif>";
			var tb=window.document.getElementById('result');
		    var source=$("#source").val();
			/*if(typeof(editor) != "undefined") {
			...
			//$("#hide_source").val(editor.getValue());
			//document.getElementById("frmSolution").submit();
			$.post("submit_fill.php?ajax",$("#frmSolution").serialize(),function(data){fresh_result(data);});
