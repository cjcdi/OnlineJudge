<?php
  ...
  header("content-type:application/javascript"); //定义js
  ...
  if(isset($_SESSION[$OJ_NAME.'_'.'user_id'])){ //登录状态下
    $sid=$_SESSION[$OJ_NAME.'_'.'user_id'];

    $profile.= "<li><a href=".$path_fix."modifypage.php>$MSG_REG_INFO</a></li>&nbsp;<li><a href='".$path_fix."userinfo.php?user=$sid'><span id=red>$MSG_USERINFO</span></a></li>";//设置注册信息和用户信息
    ...
    $profile.= "<li><a href=".$path_fix."loginpage.php>$MSG_LOGIN</a></li>&nbsp;"; //登陆

    if($OJ_LOGIN_MOD=="hustoj"){
      if(isset($OJ_REGISTER) && !$OJ_REGISTER){
      }else{
        $profile.= "<li><a href=".$path_fix."registerpage.php>$MSG_REGISTER</a></li>&nbsp;"; //注册
      }
    }
  }
