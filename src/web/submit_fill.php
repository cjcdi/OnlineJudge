...
$test_run = false;  // 提交页面是否开启了测试功能，找问题id，假如在测试模式则id是负值，并判断是否启用test_run
if (isset($_POST['id'])) {
    $id = intval($_POST['id']);
    $test_run = ($id <= 0);
...
if (isset($problem_flag) && $problem_flag == "0") {
    ...
    setcookie('lastlang', $language, time() + 360000);  // 设置 cookie
}
if ($test_run) $id = -$id;
$source = $_POST['source'];
// $input_text = "";
...
// $prepend_file = "$OJ_DATA/$id/prepend." . $language_ext[$language];  // use append Main code
...
$now = strftime("%Y-%m-%d %X", time() - 10);  // last submit
...
if ((~$OJ_LANGMASK) & (1 << $language)) {
    if (isset($problem_flag) && $problem_flag == "1") {  // 将问题解决添加入数据库
        ...
        $now_time = strftime("%Y-%m-%d %X", time());
        if ($is_right) {
            if (!isset($pid)) {
                $sql = "INSERT INTO solution(problem_id,user_id,in_date,ip,code_length,result) VALUES(?,?,?,?,?,4)";
                $insert_id = pdo_query($sql, $id, $user_id, $now_time, $ip, $len);
            } else {
                $sql = "INSERT INTO solution(problem_id,user_id,in_date,ip,code_length,contest_id,num,result) VALUES(?,?,?,?,?,?,?,4)";
                ...
                $insert_id = pdo_query($sql, $id, $user_id, $now_time, $ip, $len, $cid, $pid);
            }
            ...
            if (!isset($pid)) {
                $sql = "INSERT INTO solution(problem_id,user_id,in_date,ip,code_length,result) VALUES(?,?,?,?,?,6)";
                $insert_id = pdo_query($sql, $id, $user_id, $now_time, $ip, $len);
            } else {
                $sql = "INSERT INTO solution(problem_id,user_id,in_date,ip,code_length,contest_id,num,result) VALUES(?,?,?,?,?,?,?,6)";
                if (isset($OJ_OI_1_SOLUTION_ONLY) && $OJ_OI_1_SOLUTION_ONLY) {
                    pdo_query("UPDATE solution set contest_id =0 where contest_id=? and user_id=? and num=?", $cid, $user_id, $pid);
                }
                $insert_id = pdo_query($sql, $id, $user_id, $now_time, $ip, $len, $cid, $pid);
    ...
    if (isset($problem_flag) && $problem_flag == "0") {
        if (!isset($pid)) {
            $sql = "INSERT INTO solution(problem_id,user_id,in_date,language,ip,code_length,result) VALUES(?,?,?,?,?,?,14)";
            $insert_id = pdo_query($sql, $id, $user_id, $now_time, $language, $ip, $len);
        } else {
            $sql = "INSERT INTO solution(problem_id,user_id,in_date,language,ip,code_length,contest_id,num,result) VALUES(?,?,?,?,?,?,?,?,14)";
            if (isset($OJ_OI_1_SOLUTION_ONLY) && $OJ_OI_1_SOLUTION_ONLY) {
                pdo_query("UPDATE solution set contest_id =0 where contest_id=? and user_id=? and num=?", $cid, $user_id, $pid);
            }
            $insert_id = pdo_query($sql, $id, $user_id, $now_time, $language, $ip, $len, $cid, $pid);
        }
        $sql = "INSERT INTO `solution_fill`(`solution_id`,`problem_flag`,`solution_answer`) VALUES(?,?,?)";
        pdo_query($sql, $insert_id, $problem_flag, $solution_answer);
        ...
        $sql = "INSERT INTO `source_code_user`(`solution_id`,`source`) VALUES(?,?)";  // 将代码插入数据库
    ...
    if (isset($pid)) {
        $sql_1 = "SELECT count(*) FROM `solution` WHERE `problem_id`=? AND contest_id=?";
        $r = pdo_query($sql_1, $id, $cid);
        if (count($r)) {
            $total = intval($r[0][0]);  // 用户提交数
        } else {
            $total = 0;
        }
        unset($r);
        $sql_2 = "UPDATE contest_problem set c_submit=? where problem_id=?";
        pdo_query($sql_2, $total, $id);

        $sql_3 = "SELECT count(*) FROM solution WHERE problem_id=? AND contest_id=? AND result=4";
        $r = pdo_query($sql_3, $id);
        if (count($r)) {
            $ac = intval($r[0][0]);  // AC 数
        } else {
            $ac = 0;
        }
        unset($r);
        $sql_4 = "UPDATE contest_problem set c_accepted=? where problem_id=?";
        pdo_query($sql_4, $ac, $id);
    }
    // if ($test_run) {
    ...
    pdo_query($sql, $total, $id);

    $sql = "SELECT count(*) FROM solution WHERE problem_id=? AND result=4";
    $r = pdo_query($sql, $id);
    if (count($r)) {
        $ac = intval($r[0][0]);  // AC 数
    } else {
    ...
    if ($OJ_REDIS && $is_ok) {  // using redis task queue
        $redis = new Redis();
