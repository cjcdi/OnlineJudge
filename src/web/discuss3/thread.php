...
<center>
	...
	<div style="text-align:left;font-size:80%;float:left;">
		[ <a href="newpost.php<?php if ($cid) echo "?cid=$cid&pid=".$row['pid']; ?>">New Thread</a> ]
	</div>
	<?php if ($isadmin){ ?>
		<div style="font-size:80%; float:right"> Change sticky level to
			<?php $adminurl = "threadadmin.php?target=thread&tid={$tid}&action=";
		...
		<tr align=center class='toprow'>
			<td style="text-align:left">
				<a href="discuss.php<?php if ($row['pid']!=0 && $row['cid']!=null) echo "?pid=".$row['pid']."&cid=".$row['cid'];
					...
					<?php if ($row['pid']!=0) echo "Problem $pid"; else echo "MainBoard";?>
				</a> 
				>> <?php echo nl2br(htmlentities($row['title'],ENT_QUOTES,"UTF-8")); //nl2br — 在字符串所有新行之前插入 HTML 换行标记?>
			</td>
		</tr>
		...
		<?php
			...
			foreach ($result as $row){
				...
				<tr align=center class='<?php echo ($cnt=!$cnt)?'even':'odd';?>row'>
					<td>
						<div style="display:inline;text-align:left; float:left; margin:0 10px">
							<a href="../userinfo.php?user=<?php echo $row['author_id']?>"><?php echo $row['author_id'];?> 
							</a> @ <?php echo $row['time']; ?>
						</div>
						<div class="mon" style="display:inline;text-align:right; float:right">
							<?php if (isset($_SESSION[$OJ_NAME.'_'.'administrator'])) {?>  
								<span>[ 
									<a href=<?php if ($row['status']==0) echo "\"".$url."disable\">Disable";
												else echo "\"".$url."resume\">Resume";?> 
									</a> 
								]</span>
								<span>[ 
									<a onclick="reply(<?php echo $row['rid'];?>);">Reply
									</a> 
								]</span> 
							<?php } ?>
							...
							<span>[ 
								<a <?php if ($isuser || $isadmin) echo "href=".$url."delete";?>>Delete
								</a> 
							]</span>
	...
	<?php if (isset($_SESSION[$OJ_NAME.'_'.'user_id'])){ ?>
		...
		<div><textarea id="replyContent" name=content style="border:1px dashed #8080FF; width:700px; height:200px; font-size:75%;margin:0 10px; padding:10px"></textarea>
		</div>
