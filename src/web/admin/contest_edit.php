...
<body leftmargin="30">
    <?php
    if (isset($_POST['do']) && $_POST['do'] == "editc") {
        ...
        $cp_result = pdo_query($sql, $cid);  // 备份原竞赛赛题数据
        foreach ($cp_result as $v) {
            $cp_all[$v['problem_id']] = $v;
        }
        $sql = "DELETE FROM `contest_problem` WHERE `contest_id`=?";
        pdo_query($sql, $cid);  // 删除原竞赛赛题数据
        if (count($pieces) > 0 && strlen($pieces[0]) > 0) {
            $sql_1 = "INSERT INTO `contest_problem`(`contest_id`,`problem_id`,`num`) VALUES (?,?,?)";
            $sql_2 = "INSERT INTO `contest_problem`(`contest_id`,`problem_id`,`num`,`c_accepted`,`c_submit`) VALUES (?,?,?,?,?)";
            for ($i = 0; $i < count($pieces); $i++) {
                if (isset($cp_all[intval($pieces[$i])])) {  // 原竞赛赛题中有数据，则存入
                    pdo_query($sql_2, $cid, intval($pieces[$i]), $i, $cp_all[intval($pieces[$i])][c_accepted], $cp_all[intval($pieces[$i])][c_submit]);
                } else {
                    pdo_query($sql_1, $cid, intval($pieces[$i]), $i);
                }
            }
            pdo_query("update solution set num=-1 where contest_id=?", $cid);  // 所以有可能解决表里面有些 num 时 -1
            $plist = "";
            for ($i = 0; $i < count($pieces); $i++) {
                if ($plist) $plist .= ",";
                $plist .= $pieces[$i];
                $sql_3 = "update solution set num=? where contest_id=? and problem_id=?;";
                pdo_query($sql_3, $i, $cid, $pieces[$i]);
            }
        ...
        $pieces = explode("\n", trim($_POST['ulist']));
        if (count($pieces) > 0 && strlen($pieces[0]) > 0) {
            $sql_4 = "INSERT INTO `privilege`(`user_id`,`rightstr`) VALUES (?,?)";
            for ($i = 0; $i < count($pieces); $i++) {
                pdo_query($sql_4, trim($pieces[$i]), "c$cid");
            }
        }
    ...
    <div class="container">
        <form method=POST>
            ...
            <input type=hidden name=do value="editc">
