<?php
				...
				if(!strstr($file,"sae-dir-tag")){
					...
					if (isset($pinfo ['extension']) && $pinfo ['extension'] == "in" && $pinfo ['basename'] != "sample.in"){
						...
						if( $store->fileExists ("data",$infile)){
							echo "		<test_input>
			<![CDATA[".fixcdata($store->read ("data",$infile))."]]>
		</test_input>\n";
						}
						if($store->fileExists ("data",$outfile)){
							echo "		<test_output>
			<![CDATA[".fixcdata($store->read ("data",$outfile))."]]>
		</test_output>\n";
						}
		//			break;
					}     
			...
			while ( $file = readdir ( $pdir ) ) { //readdir — 从目录句柄中读取条目
				$pinfo = pathinfo ( $file );
				if (isset($pinfo ['extension']) && $pinfo ['extension'] == "in" && $pinfo ['basename'] != "sample.in") {
					$ret = basename ( $pinfo ['basename'], "." . $pinfo ['extension'] );
					$outfile="$OJ_DATA/$pid/" . $ret . ".out";
					$infile="$OJ_DATA/$pid/" . $ret . ".in";
					if(file_exists($infile)){ //file_get_contents — 将整个文件读入一个字符串
						echo "		<test_input>
			<![CDATA[".fixcdata(file_get_contents ($infile))."]]>
		</test_input>\n";
					}
					if(file_exists($outfile)){
						echo "		<test_output>
			<![CDATA[".fixcdata(file_get_contents ($outfile))."]]>
		</test_output>\n";
					}
		// break;
				}
	...
	function image_base64_encode($img_url){
		...
		$handle = @fopen($img_url, "rb"); //打开该路径的图片，将图片进行缓存
		if($handle){
	...
	function getImages($content){
		//preg_match_all — 执行一个全局正则表达式匹配，提取html中图片路径
	...
	function fixImageURL(&$html,&$did){
	   ...
	   $imgs=array_unique($images[1]); //array_unique — 移除数组中重复的值
	   foreach($imgs as $img){
	...
	if (isset($_POST ['do'])||isset($_GET['cid'])) {
	    if(isset($_POST ['in']) && strlen($_POST ['in'])>0){
			require_once("../include/check_post_key.php");
		   	$in= $_POST ['in'] ;
			$ins=explode(",",$in); //explode — 使用一个字符串分割另一个字符串
			$in="";
			foreach($ins as $pid){
				$pid=intval($pid);
				if($in) $in.=",";
...
<fps version="1.2" url="127.0.0.1">
	<generator name="Admin" url="127.0.0.1"/>
